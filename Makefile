...
.PHONY: all test test-input test-lexer test-parser test-alias test-state test-expand test-fields test-execute test-pipeline test-context test-runtime test-runtime-pty test-portability test-redirection-offset test-pty test-traps test-harness docker-build docker-test docker-test-pty docker-shell clean
...
build/tests/state_fixture: tests/state_fixture.c build/state.o build/alias.o $(INPUT_OBJECTS) $(STATE_HEADERS)
	mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSHELL_CPPFLAGS) $(CFLAGS) $(LDFLAGS) -o $@ tests/state_fixture.c build/state.o build/alias.o $(INPUT_OBJECTS) $(LDLIBS)
...
build/tests/state_faults: tests/state_faults.c tests/state_faults.h build/tests/fault-state.o build/alias.o $(STATE_HEADERS)
	mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSHELL_CPPFLAGS) $(CFLAGS) $(LDFLAGS) -o $@ tests/state_faults.c build/tests/fault-state.o build/alias.o $(LDLIBS)
...
build/execute.o build/prepare.o build/redirect.o build/builtin.o build/utility.o: build/%.o: src/%.c $(EXECUTE_HEADERS)
	mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSHELL_CPPFLAGS) $(CFLAGS) -c $< -o $@
...
.PHONY: test-syntax test-invocation
test-invocation: cshell
	$(PYTHON) tests/invocation.py ./cshell

test-syntax: cshell build/tests/syntax.json
	$(PYTHON) tests/smoke.py ./cshell --suite build/tests/syntax.json
	$(PYTHON) tests/invocation.py ./cshell

build/tests/redirection_offset_helper: tests/redirection_offset_helper.c
	mkdir -p $(dir $@)
