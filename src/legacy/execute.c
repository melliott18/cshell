/* Transitional executor. Its known behavior defects are tracked in docs/tickets/. */
#include <errno.h>
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
...
static int redirect_output(char **args, int i, int option) {
    int outfd;
    int status;
    int pid = fork();
...
static int shell_pipe(int argc, char **args, int i, int option) {
    int pipefd[3];
    pipe(pipefd);
    int pid1, pid2;
