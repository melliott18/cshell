...
identity={'recorded_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
 ...
 'source_note':'Source digest covers Makefile, Dockerfile, src, include, tests and tools, excluding Python caches. Subsequent evidence documentation is outside that digest.',
 'source_sha256':hashlib.sha256(json.dumps(manifest,sort_keys=True).encode()).hexdigest(),
 'binary':{'path':str(root/os.environ.get('EVIDENCE_BINARY', 'cshell')),'realpath':str((root/os.environ.get('EVIDENCE_BINARY', 'cshell')).resolve()),'sha256':sha(root/os.environ.get('EVIDENCE_BINARY', 'cshell'))},
