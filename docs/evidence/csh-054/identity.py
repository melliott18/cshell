...
identity={'recorded_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
 ...
 'harness_environment_names':sorted(os.environ),
 'harness_environment':{k:os.environ[k] for k in ('PATH','LANG','LC_ALL','TZ','MallocNanoZone','ASAN_OPTIONS','UBSAN_OPTIONS') if k in os.environ},
 'generated_suites':{str(p.relative_to(root)):sha(p) for p in sorted((root/'build/tests').glob('*.json'))},
 'helpers':{str(p):sha(p) for p in [pathlib.Path('/bin/cat'),pathlib.Path('/usr/bin/cmp'),root/'build/tests/execute_helper',root/'build/tests/wait_handshake',pathlib.Path('/bin/sh'),pathlib.Path('/bin/ps')] if p.exists()}}
if sys.platform=='darwin':
 identity['os_build']=command('sw_vers');identity['system_library']=command('ls','-l','/usr/lib/libSystem.B.dylib')
else:
 identity['packages']=command('dpkg-query','-W','libc6','gcc','python3','procps');
