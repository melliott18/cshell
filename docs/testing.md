...
## Execution clause evidence

`make test-execution-evidence` runs the 248 CSH-049 cases generated by
`tests/execution_cases.py` through the shared bounded runner. They also run in
the default runtime suite (`make test`), Docker and existing sanitizer CI.
The [clause map](execution-evidence.md) distinguishes required assertions from
implementation choices and links existing API, control, PTY, locale and offset
evidence. `make test-execute test-pipeline test-context test-control
 test-redirection-offset test-portability` retains the focused module paths.

`tests/fixtures/execution-known-gaps.json` is a separate strict reproducer for
[CSH-055](tickets/CSH-055-execution-contract-gaps.md); it currently **fails**. Run
